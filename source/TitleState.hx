...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		bgTitle = new FlxSprite().loadGraphic(Paths.image('backgroundTitle'));
		...
		dadDance.animation.addByPrefix('dadIdle', 'Dad idle dance', 24);
		...
		momDance.flipX = true;
		momDance.antialiasing = true;
		momDance.animation.addByPrefix('momIdle', 'Mom Idle', 24);
		...
		picoDance.animation.addByPrefix('picoIdle', 'Pico Idle Dance', 24);
		...
		spookyDance.animation.addByPrefix('spookyIdle', 'spooky dance idle', 24);
		spookyDance.animation.play('spookyIdle');
		spookyDance.updateHitbox();

		gfDance = new FlxSprite(210, 210);
		gfDance.frames = Paths.getSparrowAtlas('gfDanceTitle');
		gfDance.scale.set(0.55, 0.55);
		...
		if (pressedEnter && !transitioning && skippedIntro)
		{
			...
			new FlxTimer().start(2, function(tmr:FlxTimer)
			{
				...
				http.onData = function (data:String)
				{
					...
					{
						FlxTween.tween(logoBl, {x:275, y: -500}, 1, {type: FlxTweenType.PERSIST, ease: FlxEase.backIn});
						FlxTween.tween(titleText, {x:100, y: -300, alpha: 0}, 1, 
							{
	...
	override function beatHit()
	{
		...
		{
			case 0:
				createCoolText(['ZenusPurity_', 'rE_Do', 'TheLastHope', 'Klutch','presents']);
			case 2:
				deleteCoolText();
			case 3:
				if (Main.watermarks)
			...
			case 8:
				deleteCoolText();
			case 10:
				createCoolText(['Goodluck']);
			case 12: 
				addMoreText('You will need it');
			case 14: 
				deleteCoolText();
			case 16: 
